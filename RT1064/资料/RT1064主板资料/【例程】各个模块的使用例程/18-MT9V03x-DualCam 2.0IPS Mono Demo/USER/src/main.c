 ...
 * @note		
					...
					------------------------------------  
						摄像头FLEXIO模块管脚	单片机管脚
						SDA(51的RX)         	查看SEEKFREE_MT9V03X.h文件中的MT9V03X_COF_UART_TX	宏定义
						SCL(51的TX)         	查看SEEKFREE_MT9V03X.h文件中的MT9V03X_COF_UART_RX	宏定义
						场中断(VSY)         	查看SEEKFREE_MT9V03X.h文件中的MT9V03X_VSYNC_PIN		宏定义
						行中断(HREF)			查看SEEKFREE_MT9V03X.h文件中的MT9V03X_HREF_PIN		宏定义
						像素中断(PCLK)      	查看SEEKFREE_MT9V03X.h文件中的MT9V03X_PCLK_PIN		宏定义
						数据口(D0-D7)			查看SEEKFREE_MT9V03X.h文件中的MT9V03X_DATA_PIN		宏定义
						默认分辨率是        	188*120
					------------------------------------ 
						摄像头CSI模块管脚       单片机管脚
...
int main(void)
{
	...
	mt9v03x_csi_init();									// 初始化CSI接口总钻风摄像头
	...
	mt9v03x_init();										// 初始化FlexIO接口总钻风摄像头
    ...
    while (1)
    {
		...
		if(!key_flag && mt9v03x_csi_finish_flag)		// CSI摄像头采集完成
		{
			mt9v03x_csi_finish_flag = 0;				// 清除采集完成标志位
			ips200_displayimage032_zoom1(mt9v03x_csi_image[0],MT9V03X_CSI_W,MT9V03X_CSI_H,0,0,320,240);	// 显示图像
		}
		if(key_flag && mt9v03x_finish_flag)				// FlexIO摄像头采集完成
		{
			mt9v03x_finish_flag = 0;					// 清除采集完成标志位
			ips200_displayimage032_zoom1(mt9v03x_image[0],MT9V03X_CSI_W,MT9V03X_CSI_H,0,0,320,240);		// 显示图像
		}
