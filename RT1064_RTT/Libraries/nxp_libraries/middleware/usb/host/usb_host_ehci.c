                                              ...
                                              usb_host_ehci_pipe_t *ehciPipePointer)
{
    ...
    for (frameIndex = ehciPipePointer->startFrame; frameIndex < USB_HOST_CONFIG_EHCI_FRAME_LIST_SIZE;
         frameIndex += ((ehciPipePointer->uframeInterval + 7) / 8))
    {
...
static void USB_HostEhciPortChange(usb_host_ehci_instance_t *ehciInstance)
{
    ...
    if (portScRegister & USBHS_PORTSC1_CCS_MASK) /* process attach */
    {
        ...
        /* process attach */
        OSA_EventSet(ehciInstance->taskEventHandle, EHCI_TASK_EVENT_DEVICE_ATTACH);
        /* gpt timer start */
    ...
    else
    {
            ...
            (ehciInstance->deviceAttached == kEHCIDeviceAttached))
        {
            ...
            USB_HostEhciStopAsync(ehciInstance);
            USB_HostEhciStopPeriodic(ehciInstance);
            OSA_EventSet(ehciInstance->taskEventHandle, EHCI_TASK_EVENT_DEVICE_DETACH);
        }
    }
}
...
#endif
        OSA_MutexDestroy(ehciInstance->ehciMutex);
        OSA_MemoryFree(ehciInstance);
        return kStatus_USB_Error;
    }
...
#endif
    OSA_MutexDestroy(ehciInstance->ehciMutex);
    OSA_EventDestroy(ehciInstance->taskEventHandle);
    OSA_MemoryFree(ehciInstance);
...
void USB_HostEhciIsrFunction(void *hostHandle)
{
    ...
    while (interruptStatus) /* there are usb interrupts */
    {
        ...
        {
            OSA_EventSet(ehciInstance->taskEventHandle, EHCI_TASK_EVENT_TRANSACTION_DONE);
        }
...
#endif
            OSA_EventSet(ehciInstance->taskEventHandle, EHCI_TASK_EVENT_PORT_CHANGE);
        }

        if (interruptStatus & USBHS_USBSTS_TI0_MASK) /* timer 0 interrupt */
        {
            OSA_EventSet(ehciInstance->taskEventHandle, EHCI_TASK_EVENT_TIMER0);
        }

#if ((defined(USB_HOST_CONFIG_LOW_POWER_MODE)) && (USB_HOST_CONFIG_LOW_POWER_MODE > 0U))
        if (interruptStatus & USBHS_USBSTS_TI1_MASK) /* timer 1 interrupt */
        {
            OSA_EventSet(ehciInstance->taskEventHandle, EHCI_TASK_EVENT_TIMER1);
        }
#endif
