...
usb_phydcd_status_t USB_PHYDCD_Init(uint8_t index, usb_phydcd_config_struct_t *config, usb_phydcd_handle *dcdHandle)
{
    ...
    dcdState->phyBase = (void *)phyBase[index + 1];
    ...
    dcdState->dcdDetectState = kUSB_DCDDetectInit;
    *dcdHandle = dcdState;
    return dcdError;
}
...
usb_phydcd_status_t USB_PHYDCD_Control(usb_phydcd_handle handle, usb_phydcd_control_t type, void *param)
{
    ...
    switch (type)
    {
        case kUSB_DevicePHYDcdRun:
            ...
            {
                dcdState->dcdDetectState = kUSB_DCDDetectStart;
            }
            break;
...
usb_phydcd_status_t USB_PHYDCD_TimerIsrFunction(usb_phydcd_handle handle)
{
    ...
    switch (dcdState->dcdDetectState)
    {
        ...
        case kUSB_DCDDetectStart:
            ...
            dcdState->dcdDetectState = kUSB_DCDDataContactDetection;
            dcdState->detectResult = kUSB_DcdUnknownType;
            ...
            dcdState->usbAnalogBase->INSTANCE[dcdState->index].CHRG_DETECT_SET |= USB_ANALOG_CHRG_DETECT_SET_CHK_CONTACT_MASK | USB_ANALOG_CHRG_DETECT_SET_CHK_CHRG_B_MASK;
        ...
        case kUSB_DCDDataContactDetection:
            ...
            {
                if(dcdState->usbAnalogBase->INSTANCE[dcdState->index].CHRG_DETECT_STAT & USB_ANALOG_CHRG_DETECT_STAT_PLUG_CONTACT_MASK)
                {
                    dcdState->dataPinCheckTimes++;
                    if(dcdState->dataPinCheckTimes >= 5)
                    {
                        dcdState->dcdDetectState = kUSB_DCDPrimaryDetection;
                        dcdState->usbAnalogBase->INSTANCE[dcdState->index].CHRG_DETECT_CLR |= USB_ANALOG_CHRG_DETECT_CLR_CHK_CONTACT_MASK | USB_ANALOG_CHRG_DETECT_CLR_CHK_CHRG_B_MASK;
                        dcdState->startTime = dcdState->hwTick;
                    }
                }
                else
                {
                    dcdState->dataPinCheckTimes = 0;
                }
            }

            if((dcdState->hwTick - dcdState->startTime)>= USB_DCD_DATA_PIN_DETECTION_TIME*USB_DCD_DATA_PIN_MAX_DETECTION_COUNT)
            {
                if(kUSB_DCDDataContactDetection == dcdState->dcdDetectState)
                {
                    dcdState->dcdDetectState = kUSB_DCDDetectInit;
                    dcdState->startTime = 0U;
                    dcdState->usbAnalogBase->INSTANCE[dcdState->index].CHRG_DETECT_SET |= USB_ANALOG_CHRG_DETECT_SET_EN_B_MASK | USB_ANALOG_CHRG_DETECT_SET_CHK_CHRG_B_MASK;
                    dcdState->detectResult = kUSB_DcdError;
                    dcdState->dcdDetectState = kUSB_DCDDectionFinished;
                }
            }
            break;
        case kUSB_DCDPrimaryDetection:
            ...
            {
                if (!(dcdState->usbAnalogBase->INSTANCE[dcdState->index].CHRG_DETECT_STAT & USB_ANALOG_CHRG_DETECT_STAT_CHRG_DETECTED_MASK))
                {
                    dcdState->detectResult = kUSB_DcdSDP;
                    dcdState->dcdDetectState = kUSB_DCDDectionFinished;
                }
                else
                {
                    dcdState->dcdDetectState = kUSB_DCDSecondaryDetection;
                }
                dcdState->usbAnalogBase->INSTANCE[dcdState->index].CHRG_DETECT_SET |= USB_ANALOG_CHRG_DETECT_SET_EN_B_MASK | USB_ANALOG_CHRG_DETECT_SET_CHK_CHRG_B_MASK;
                if(kUSB_DCDSecondaryDetection == dcdState->dcdDetectState)
                {
                    usbPhyBase = (USBPHY_Type *)dcdState->phyBase;
                    usbPhyBase->DEBUG_CLR |= USBPHY_DEBUG_CLR_CLKGATE_MASK;
                    dcdState->usbAnalogBase->INSTANCE[dcdState->index].LOOPBACK_SET |= USB_ANALOG_LOOPBACK_UTMI_TESTSTART_MASK;
                }
                dcdState->startTime = dcdState->hwTick;
            }
            break;
        case kUSB_DCDSecondaryDetection:
            ...
            {
                if(dcdState->usbAnalogBase->INSTANCE[dcdState->index].CHRG_DETECT_STAT & USB_ANALOG_CHRG_DETECT_STAT_DM_STATE_MASK)
                {
                    dcdState->detectResult = kUSB_DcdDCP;
                }
                else
                {
                    dcdState->detectResult = kUSB_DcdCDP;
                }
        ...
        case kUSB_DCDDectionFinished:
            dcdState->dcdDetectState = kUSB_DCDDetectIdle;
            dcdState->dcdCallback(dcdState->dcdCallbackParam, dcdState->detectResult, (void *)&dcdState->detectResult);
            break;
        default:
            break;
    }
