...
net_path = "last-mobilenet_v2-0.00005-25-450.tflite"                                  # 定义模型的路径
...
while(True):
    while(uart.any()):
        ...
        for r in img.find_rects(threshold = 80000):             # 在图像中搜索矩形
            ...
            result = net.classify(img1)[0].output()
            max_score = max(result)
            max_label = result.index(max_score)
            print(max_label)
            uart_array = bytearray([0XFF,int(max_label)])
